...
#[command]
#[only_in(dms)]
pub async fn truesight(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if let Some(p) = player {
        if let LupusRole::VEGGENTE = *p.role() {
            LupusCtxHelper::send_lupus_command(ctx, msg, LupusAction::TrueSight(target_id)).await?
        } else {
            msg.channel_id
